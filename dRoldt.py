# -*- coding: utf-8 -*-
"""
Created on Tue Jan 30 16:15:53 2024
...
model=Sequential()
model.add(Dense(64, input_dim=2, kernel_initializer='normal', activation=tf.keras.layers.LeakyReLU(alpha=0.01), name= 'input'))
...
sgd = optimizers.SGD(learning_rate=0.01, decay=2e-8, momentum=0.9, nesterov=True, clipvalue=0.5)
...
Xnew = np.array([4.11E+00,	17300]).reshape(1, -1)
Xnew= scaler_x.transform(Xnew)
ynew= model.predict(Xnew)
ynew = scaler_y.inverse_transform(ynew)
Xnew = scaler_x.inverse_transform(Xnew)
print("X=%s, Predicted=%s" % (Xnew, ynew),'real y = 1.33e-2')
Xnew1 = np.array([2.09E+01,	17300]).reshape(1, -1)
...
print("X=%s, Predicted=%s" % (Xnew1, ynew1),'real y = 6.78E-02')

weights = model.layers[0].get_weights()[1]
np.savetxt('dRoldt_bias-input_layer.txt', weights, fmt='%1.4e', delimiter=',')
weights1 = model.layers[0].get_weights()[0]
np.savetxt('dRoldt_weights-input_layer.txt', weights1, fmt='%1.4e', delimiter=',')

weights2 = model.layers[1].get_weights()[1]
np.savetxt('dRoldt_bias-first_layer.txt', weights2, fmt='%1.4e', delimiter=',')
weights3 = model.layers[1].get_weights()[0]
np.savetxt('dRoldt_weights-first_layer.txt', weights3, fmt='%1.4e', delimiter=',')

weights4 = model.layers[2].get_weights()[1]
np.savetxt('dRoldt_bias-second_layer.txt', weights4, fmt='%1.4e', delimiter=',')
weights5 = model.layers[2].get_weights()[0]
np.savetxt('dRoldt_weights-second_layer.txt', weights5, fmt='%1.4e', delimiter=',')

weights6 = model.layers[3].get_weights()[1]
np.savetxt('dRoldt_bias-third_layer.txt', weights6, fmt='%1.4e', delimiter=',')
weights7 = model.layers[3].get_weights()[0]
np.savetxt('dRoldt_weights-third_layer.txt', weights7, fmt='%1.4e', delimiter=',')
